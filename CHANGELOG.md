...
### 构建
- 移除package.json中的prepublishOnly脚本，该脚本在发布前执行清理、构建和测试
- 移除deploy.yml中的always-auth配置，该配置不再需要

## [1.1.2] - 2025-12-20

### 构建
- 移除不再需要的 `nest-cli.json` 配置文件
- 将构建命令从 `nest build` 改为 `tsc` 直接编译

## [1.1.1] - 2025-12-20

### 修复
- 修复构建输出与包入口不一致问题：产物统一为 `dist/index.js` 与 `dist/index.d.ts`
- 限定构建范围为 `src` 并设置 `rootDir: src`，避免生成 `dist/src` 层级

### 优化
- 在 `.gitignore` 中忽略 `*.tsbuildinfo` 缓存文件
